...
static int bfusb_rx_submit(struct bfusb_data* data, struct urb* urb) {
  ...
  usb_fill_bulk_urb(urb, data->udev, pipe, skb->data, size, bfusb_rx_complete,
                    skb);
  ...
  err = usb_submit_urb(urb, GFP_ATOMIC);
  if (err) {
    bt_dev_err(data->hdev, "bulk rx submit failed urb %p err %d", urb, err);
    skb_unlink(skb, &data->pending_q);
    kfree_skb(skb);
    usb_free_urb(urb);
  }
...
static inline int bfusb_recv_block(struct bfusb_data* data,
                                   int hdr,
                                   unsigned char* buf,
                                   int len) {
  ...
  if (hdr & 0x04) {
    ...
    switch (pkt_type) {
      ...
      default:
        bt_dev_err(data->hdev, "unknown packet type 0x%02x", pkt_type);
        return -EILSEQ;
    }
...
static void bfusb_rx_complete(struct urb* urb) {
  ...
  while (count) {
    ...
    if ((hdr & 0xe1) == 0xc1 && bfusb_recv_block(data, hdr, buf, len) < 0)
...
resubmit:
  ...
  err = usb_submit_urb(urb, GFP_ATOMIC);
  if (err) {
    bt_dev_err(data->hdev, "bulk resubmit failed urb %p err %d", urb, err);
  }
...
static int bfusb_send_frame(struct hci_dev* hdev, struct sk_buff* skb) {
  ...
  BT_DBG("hdev %p skb %p type %d len %d", hdev, skb, hci_skb_pkt_type(skb),
         skb->len);
...
static int bfusb_load_firmware(struct bfusb_data* data,
  ...
  if (usb_control_msg(data->udev, pipe, USB_REQ_SET_CONFIGURATION, 0, 1, 0,
  ...
  while (count) {
    ...
    err = usb_bulk_msg(data->udev, pipe, buf, size, &len, BFUSB_BLOCK_TIMEOUT);
  ...
  err = usb_bulk_msg(data->udev, pipe, NULL, 0, &len, BFUSB_BLOCK_TIMEOUT);
  if (err < 0) {
  ...
  err = usb_control_msg(data->udev, pipe, USB_REQ_SET_CONFIGURATION, 0, 2, 0,
...
error:
  ...
  usb_control_msg(data->udev, pipe, USB_REQ_SET_CONFIGURATION, 0, 0, 0, NULL, 0,
