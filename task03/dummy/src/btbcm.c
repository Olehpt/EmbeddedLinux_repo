...
static int btbcm_set_bdaddr_from_efi(struct hci_dev* hdev) {
  efi_guid_t guid = EFI_GUID(0x74b00bd9, 0x805a, 0x4d61, 0xb5, 0x1f, 0x43, 0x26,
                             0x81, 0x23, 0xd1, 0x13);
...
int btbcm_patchram(struct hci_dev* hdev, const struct firmware* fw) {
  ...
  if (IS_ERR(skb)) {
    err = PTR_ERR(skb);
    bt_dev_err(hdev, "BCM: Download Minidrv command failed (%d)", err);
    return err;
  }
  kfree_skb(skb);
  ...
  while (fw_size >= sizeof(*cmd)) {
    ...
    skb = __hci_cmd_sync(hdev, opcode, cmd->plen, cmd_param, HCI_INIT_TIMEOUT);
    if (IS_ERR(skb)) {
      err = PTR_ERR(skb);
      bt_dev_err(hdev, "BCM: Patch command %04x failed (%d)", opcode, err);
      return err;
    }
    kfree_skb(skb);
  }
...
static struct sk_buff* btbcm_read_local_name(struct hci_dev* hdev) {
  ...
  skb = __hci_cmd_sync(hdev, HCI_OP_READ_LOCAL_NAME, 0, NULL, HCI_INIT_TIMEOUT);
  if (IS_ERR(skb)) {
    bt_dev_err(hdev, "BCM: Reading local name failed (%ld)", PTR_ERR(skb));
    return skb;
  }
...
static struct sk_buff* btbcm_read_usb_product(struct hci_dev* hdev) {
  ...
  if (IS_ERR(skb)) {
    bt_dev_err(hdev, "BCM: Read USB product info failed (%ld)", PTR_ERR(skb));
    return skb;
  }
...
int btbcm_initialize(struct hci_dev* hdev,
                     bool* fw_load_done,
  ...
  bcm_subver_table =
      (hdev->bus == HCI_USB) ? bcm_usb_subver_table : bcm_uart_subver_table;
  ...
  fw_name = kmalloc_array(BCM_FW_NAME_COUNT_MAX, sizeof(*fw_name), GFP_KERNEL);
  if (!fw_name)
    return -ENOMEM;

  if (hw_name) {
    if (board_name) {
      snprintf(fw_name[fw_name_count], BCM_FW_NAME_LEN, "brcm/%s%s.%s.hcd",
               hw_name, postfix, board_name);
      fw_name_count++;
    }
    snprintf(fw_name[fw_name_count], BCM_FW_NAME_LEN, "brcm/%s%s.hcd", hw_name,
             postfix);
    fw_name_count++;
  }

  if (board_name) {
    snprintf(fw_name[fw_name_count], BCM_FW_NAME_LEN, "brcm/BCM%s.%s.hcd",
             postfix, board_name);
    fw_name_count++;
  }
  snprintf(fw_name[fw_name_count], BCM_FW_NAME_LEN, "brcm/BCM%s.hcd", postfix);
  fw_name_count++;

  for (i = 0; i < fw_name_count; i++) {
    err = firmware_request_nowarn(&fw, fw_name[i], &hdev->dev);
    if (err == 0) {
      bt_dev_info(hdev, "%s '%s' Patch", hw_name ? hw_name : "BCM", fw_name[i]);
      *fw_load_done = true;
      break;
    }
  }
