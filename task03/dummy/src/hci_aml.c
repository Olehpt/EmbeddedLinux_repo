 ...
 */
static int aml_send_tci_cmd(struct hci_dev* hdev,
                            u16 op_code,
                            u32 op_addr,
                            u32* param,
                            u32 param_len) {
  ...
  skb = __hci_cmd_sync_ev(hdev, op_code, buf_len, buf, HCI_EV_CMD_COMPLETE,
                          HCI_INIT_TIMEOUT);
  if (IS_ERR(skb)) {
    err = PTR_ERR(skb);
  ...
  if (rsp->opcode != op_code || rsp->status != 0x00) {
    bt_dev_err(hdev, "send TCI cmd (0x%04X), response (0x%04X):(%d)", op_code,
...
static int aml_update_chip_baudrate(struct hci_dev* hdev, u32 baud) {
  ...
  return aml_send_tci_cmd(hdev, AML_TCI_CMD_UPDATE_BAUDRATE, AML_OP_UART_MODE,
                          &value, sizeof(value));
}
...
static int aml_start_chip(struct hci_dev* hdev) {
  ...
  ret = aml_send_tci_cmd(hdev, AML_TCI_CMD_WRITE, AML_OP_MEM_HARD_TRANS_EN,
                         &value, sizeof(value));
  if (ret)
    return ret;
  ...
  ret = aml_send_tci_cmd(hdev, AML_TCI_CMD_HARDWARE_RESET, AML_OP_HARDWARE_RST,
                         &value, sizeof(value));
  return ret;
}
...
static int aml_send_firmware_segment(struct hci_dev* hdev,
  ...
  return aml_send_tci_cmd(hdev, AML_TCI_CMD_DOWNLOAD_BT_FW, op_addr, (u32*)seg,
                          seg_size);
}

static int aml_send_firmware(struct hci_dev* hdev,
                             u8 fw_type,
                             u8* fw,
                             u32 fw_size,
                             u32 offset) {
  ...
  while (fw_size > 0) {
    seg_size = (fw_size > AML_FIRMWARE_OPERATION_SIZE)
                   ...
                   : fw_size;
    if (aml_send_firmware_segment(hdev, fw_type, (fw + seg_off), seg_size,
                                  offset)) {
      bt_dev_err(hdev, "Failed send firmware, type: %d, offset: 0x%x", fw_type,
                 offset);
      return -EINVAL;
    }
    seg_off += seg_size;
...
static int aml_download_firmware(struct hci_dev* hdev, const char* fw_name) {
  ...
  value = AML_EVT_EN;
  ret = aml_send_tci_cmd(hdev, AML_TCI_CMD_WRITE, AML_OP_EVT_ENABLE, &value,
                         sizeof(value));
  if (ret)
    goto exit;
  ...
  ret = aml_send_tci_cmd(hdev, AML_TCI_CMD_WRITE, AML_OP_RAM_POWER_CTR, &value,
                         sizeof(value));
  if (ret)
    goto exit;

  /* Check RAM power status */
  ret = aml_send_tci_cmd(hdev, AML_TCI_CMD_READ, AML_OP_RAM_POWER_CTR, NULL, 0);
  if (ret)
    goto exit;
  ...
  if (fw_len->iccm_len < amldev->aml_dev_data->iccm_offset ||
      ...
      fw_len->dccm_len > firmware->size - sizeof(*fw_len) - fw_len->iccm_len) {
  ...
  /* Download DCCM */
  dccm_start =
      (u8*)(firmware->data) + sizeof(struct aml_fw_len) + fw_len->iccm_len;
...
static int aml_send_reset(struct hci_dev* hdev) {
  ...
  skb = __hci_cmd_sync_ev(hdev, HCI_OP_RESET, 0, NULL, HCI_EV_CMD_COMPLETE,
                          HCI_INIT_TIMEOUT);
  if (IS_ERR(skb)) {
    err = PTR_ERR(skb);
...
static int aml_dump_fw_version(struct hci_dev* hdev) {
  ...
  if (rsp->opcode != AML_BT_HCI_VENDOR_CMD || rsp->status != 0x00) {
    bt_dev_err(hdev, "dump version, error response (0x%04X):(%d)", rsp->opcode,
               rsp->status);
    err = -EINVAL;
    goto exit;
  }
...
static int aml_set_bdaddr(struct hci_dev* hdev, const bdaddr_t* bdaddr) {
  ...
  skb = __hci_cmd_sync_ev(hdev, AML_BT_HCI_VENDOR_CMD, sizeof(bdaddr_t), bdaddr,
                          HCI_EV_CMD_COMPLETE, HCI_INIT_TIMEOUT);
  if (IS_ERR(skb)) {
    err = PTR_ERR(skb);
...
static int aml_recv(struct hci_uart* hu, const void* data, int count) {
  ...
  aml_data->rx_skb = h4_recv_buf(hu, aml_data->rx_skb, data, count,
                                 aml_recv_pkts, ARRAY_SIZE(aml_recv_pkts));
